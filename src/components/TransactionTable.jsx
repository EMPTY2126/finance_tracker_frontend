    ...
    return (
      <div className="rounded-xl border border-dashed border-border p-10 text-center">
        ...
        <p className="mt-1 text-xs text-faint">
          Add one above, or widen your filters.
        </p>
      </div>
    );
  }
        ...
        <tbody>
          {transactions.map((t) => (
            <tr key={t.id} className="border-b border-border last:border-0">
              ...
              <td
                className={`num py-3 pr-4 text-right font-medium ${t.type === "INCOME" ? "text-brand" : "text-rose"}`}
              >
                {t.type === "INCOME" ? "+" : "−"}
              ...
              <td className="py-3 pr-4 text-right">
                <button
                  onClick={() => onEdit(t)}
                  className="mr-3 text-xs font-medium text-muted hover:text-brand"
                >
                  Edit
                </button>
                <button
                  onClick={() => onDelete(t)}
                  className="text-xs font-medium text-muted hover:text-rose"
                >
                  Delete
                </button>
              </td>
